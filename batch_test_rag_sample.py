...
# -*- coding: utf-8 -*-
"""
批量测试RAG管道脚本 - Task 5 (样例测试版)
端到端集成与批量测试，仅处理前20个样例验证流程
"""
...
def main():
    print("=" * 60)
    print("Task 5: 端到端集成与批量测试 (样例测试 - 前20个)")
    print("=" * 60)
    ...
    # 加载测试数据（仅前20个）
    print(f"正在加载测试数据: {test_file} (仅前20个样例)")
    ...
    log_entries.append(f"样例测试开始时间: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    log_entries.append(f"测试数据集: {test_file}")
    log_entries.append(f"样例数量: {len(test_data)} (前20个)")
    ...
    # 添加性能统计到日志
    log_entries.extend([
        f"样例测试结束时间: {end_time.strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    # 保存submission.json文件（样例版）
    submission_file = "output/submission_sample.json"
    print(f"\n正在保存样例提交文件: {submission_file}")
    ...
    # 显示统计结果
    print("\n" + "=" * 60)
    print("Task 5 样例测试完成统计")
    print("=" * 60)
    print(f"样例测试数量: {len(results)}")
    ...
    print(f"\n生成文件:")
    print(f"  - 样例提交文件: {submission_file}")
    ...
    for file_path in files_to_check:
        if os.path.exists(file_path):
            ...
            print(f"✓ {file_path} - 文件大小: {file_size} 字节")
        else:
    ...
    if all_files_exist and successful_count > 0:
        print(f"\n🎉 样例测试流程验证成功！")
        print(f"   - 成功处理了 {successful_count}/{len(test_data)} 个问题")
        print(f"   - 所有输出文件均已正确生成")
        print(f"   - 可以继续进行完整测试")
    else:
        print(f"\n⚠️  样例测试存在问题，请检查后再进行完整测试")
